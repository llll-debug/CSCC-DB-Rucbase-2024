...
struct TabMeta {
    ...
    std::unordered_map<std::string, IndexMeta> indexes; // 表上建立的索引，使用unordered_map以便快速查找
    ...
    bool is_index(const std::vector<std::string>& col_names) {
        std::string index_name = get_index_name(col_names);
        // 使用哈希表快速判断
        return indexes.count(index_name) != 0;
    }

    /* 根据字段名称集合获取索引元数据 */
    IndexMeta get_index_meta(const std::vector<std::string>& col_names) {
        auto &&index_meta = indexes.find(get_index_name(col_names));
        if (index_meta != indexes.end()) {
            return index_meta->second;
        }
