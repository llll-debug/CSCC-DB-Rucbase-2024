...
class InsertExecutor : public AbstractExecutor {
 ...
 public:
  ...
  InsertExecutor(SmManager* sm_manager, std::string tab_name,
  ...
  std::unique_ptr<RmRecord> Next() override {
    ...
    for (auto& [index_name, index] : tab_.indexes) {
      ...
      for (auto& [index_offset, col_meta] : index.cols) {
        memcpy(keys[i] + index_offset, rec.data + col_meta.offset,
               col_meta.len);
      }
      RmRecord rm_record(keys[i], index.col_tot_len);
      not_inserted = context_->lock_mgr_->isSafeInGap(context_->txn_, index,
                                                      rm_record, fh_->GetFd());
    ...
    //         v.raw = std::make_shared<RmRecord>(rec.data + col_meta.offset,
    //         col_meta.len); conds[idx].op = OP_EQ; conds[idx].lhs_col = {"",
    //         col_meta.name}; conds[idx].rhs_val = std::move(v);
    //         ++idx;
    //     }
    ...
    //     // for (auto &[index_offset, col_meta]: index.cols) {
    //     //     memcpy(rm_record.data + index_offset, rec.data +
    //     col_meta.offset, col_meta.len);
    //     // }
    //     context_->lock_mgr_->lock_exclusive_on_gap(context_->txn_,
    //     index_meta, gap, fh_->GetFd());
    // }
    ...
    auto* insert_log_record = new InsertLogRecord(
    ...
    auto* write_record =
        new WriteRecord(WType::INSERT_TUPLE, rid_, rec, tab_name_);
