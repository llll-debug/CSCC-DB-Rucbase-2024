...
class SortMergeJoinExecutor : public AbstractExecutor {
 private:
  std::unique_ptr<AbstractExecutor>
      left_;  // 左儿子节点（需要join的表，已经sort）
  std::unique_ptr<AbstractExecutor>
      right_;                         // 右儿子节点（需要join的表，已经sort）
 ...
 public:
    ...
    do {
      ...
      while (!right_->is_end()) {
        ...
        if (cmp_conds(lhs_rec_.get(), rhs_rec_.get(), fed_conds_, cols_)) {
          ...
          memcpy(rm_record_->data + left_->tupleLen(), rhs_rec_->data,
                 right_->tupleLen());
          // 第一次匹配，拷贝一份
          if (++match_cnts_ == 1) {
  ...
  void nextTuple() override {
    ...
    if (is_rollback_ && rollback_cnts_ > 0) {
      ...
      // 拷贝左右元组
      memcpy(rm_record_->data, lhs_rec_->data, left_->tupleLen());
      memcpy(rm_record_->data + left_->tupleLen(), prev_rhs_rec_->data,
             right_->tupleLen());
      return;
    }
    ...
    do {
        ...
        switch (last_cond_.op) {
          case OP_EQ: {
            ...
            if (last_cmp_ < 0) {
              ...
              if (match_cnts_ > 0 &&
                  cmp_conds(lhs_rec_.get(), prev_rhs_rec_.get(), fed_conds_,
                            cols_)) {
                ...
                // 拷贝左右元组
                memcpy(rm_record_->data, lhs_rec_->data, left_->tupleLen());
                memcpy(rm_record_->data + left_->tupleLen(),
                       prev_rhs_rec_->data, right_->tupleLen());
                return;
              }
              match_cnts_ = 0;
              // // 和当前匹配
              // if (cmp_conds(lhs_rec_.get(), rhs_rec_.get(), fed_conds_,
              // cols_)) {
              //     rm_record_ = std::make_unique<RmRecord>(len_);
              //     // 拷贝左右元组
              //     memcpy(rm_record_->data, lhs_rec_->data,
              //     left_->tupleLen()); memcpy(rm_record_->data +
              //     left_->tupleLen(), rhs_rec_->data, right_->tupleLen());
              //     return;
              // }
            } else {
              match_cnts_ = 0;
      ...
      if (last_cond_.op == OP_NE) {
        ...
        // 1. 如果 prev_rhs_rec 满足谓词，要回滚
        if (match_cnts_ > 0 &&
            cmp_conds(lhs_rec_.get(), prev_rhs_rec_.get(), fed_conds_, cols_)) {
          rollback_cnts_ = match_cnts_;
          is_rollback_ = true;
          --rollback_cnts_;
          rm_record_ = std::make_unique<RmRecord>(len_);
        ...
        // assert(false);
        // match_cnts_ = 0;
        // // 判断左表新元组是否满足谓词条件，满足继续连接，否则直接 early
        // return if (cmp_conds(lhs_rec_.get(), rhs_rec_.get(), fed_conds_,
        // cols_)) {
        //     rm_record_ = std::make_unique<RmRecord>(len_);
        //     // 拷贝左右元组
        //     memcpy(rm_record_->data, lhs_rec_->data, left_->tupleLen());
        //     memcpy(rm_record_->data + left_->tupleLen(), rhs_rec_->data,
        //     right_->tupleLen()); return;
        // }
  ...
  bool is_end() const {
    return left_->is_end() || is_right_empty_ || early_end_;
  }
  ...
  static inline int compare(const char* a, const char* b, int col_len,
                            ColType col_type) {
    switch (col_type) {
      case TYPE_INT: {
  ...
  // 判断是否满足单个谓词条件
  bool cmp_cond(const RmRecord* lhs_rec, const RmRecord* rhs_rec,
                const Condition& cond, const std::vector<ColMeta>& rec_cols) {
    last_cond_ = cond;
    ...
    if (cond.is_rhs_val) {
      ...
      rhs_data = rhs_rec->data + rhs_col_meta->offset -
                 left_->tupleLen();  // 记得减去左边的 tuplelen
    }
    ...
    last_cmp_ =
        compare(lhs_data, rhs_data, lhs_col_meta->len, lhs_col_meta->type);
    switch (cond.op) {
      case OP_EQ:
        return last_cmp_ == 0;
      case OP_NE:
  ...
  bool cmp_conds(const RmRecord* lhs_rec, const RmRecord* rhs_rec,
