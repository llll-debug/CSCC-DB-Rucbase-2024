...
Rid RmFileHandle::insert_record(char* buf, Context* context) {
  ...
  // 行级 X 锁
  // if (context != nullptr) {
  //     context->lock_mgr_->lock_exclusive_on_record(context->txn_,
...
void RmFileHandle::insert_record(const Rid& rid, char* buf) {
  ...
  // if (context != nullptr) {
  //     context->lock_mgr_->lock_exclusive_on_record(context->txn_,
  //     {page_handle.page->get_page_id().page_no, slot_no}, fd_);
  // }
  auto page_handle = fetch_page_handle(rid.page_no);
  page_handle.page->WLatch();
  if (!Bitmap::is_set(page_handle.bitmap, rid.slot_no)) {
 ...
 * @param {char*} buf 要插入记录的数据
 */
void RmFileHandle::load_record(int& page_no, char*& data, int nums_record,
                               int page_size) {
  PageId page_id{fd_, page_no};
  auto page_handle =
      RmPageHandle{&file_hdr_, buffer_pool_manager_->new_page(&page_id)};
