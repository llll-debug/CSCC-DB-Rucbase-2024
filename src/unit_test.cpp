...
const std::string TEST_DB_NAME =
    "BufferPoolManagerTest_db";                         // 以数据库名作为根目录
...
void check_cache(int fd, int page_no) {
  ...
  char* mock_buf =
      mock_get_page(fd, page_no);  // &mock[fd][page_no * PAGE_SIZE];
...
void check_equal(
  ...
  for (auto& entry : mock) {
    ...
    assert(memcmp(mock_buf, rec->data, file_handle->file_hdr_.record_size) ==
           0);
  }
  // Randomly get record
  for (int i = 0; i < 10; i++) {
...
class BigStorageTest : public ::testing::Test {
 ...
 public:
  ...
  void TearDown() override {
    ...
    // disk_manager_->destroy_file(TEST_FILE_NAME_BIG);  // you can choose to
    // delete the file
...
// 对DiskManager测试文件创建和删除函数，读写页面需要先实现buffer pool类
TEST(DiskManagerTest, SimpleTest) {
  ...
  for (size_t i = 0; i < filenames.size(); i++) {
    ...
    disk_manager->set_fd2pageno(
        fd, 0);  // diskmanager在fd对应的文件中从0开始分配page_no
  }
  ...
  for (auto& entry : fd2name) {
    ...
    // 这里如果注释了destroy_file 就能看到创建好的文件
    disk_manager->destroy_file(filename);
    try {
      disk_manager->destroy_file(filename);
      assert(false);
...
class BufferPoolManagerTest : public ::testing::Test {
 ...
 public:
  ...
  void TearDown() override {
    disk_manager_->close_file(fd_);
    // disk_manager_->destroy_file(TEST_FILE_NAME);  // you can choose to delete
    // the file
...
TEST_F(BufferPoolManagerTest, SampleTest) {
  ...
  // new_page again, and now all buffers are pinned. Page 0 would be failed to
  // fetch.
  EXPECT_NE(nullptr, bpm->new_page(&page_id_temp));
...
class BufferPoolManagerConcurrencyTest : public ::testing::Test {
 ...
 public:
  ...
  void TearDown() override {
    disk_manager_->close_file(fd_);
    // disk_manager_->destroy_file(TEST_FILE_NAME_CCUR);  // you can choose to
    // delete the file
...
TEST_F(BufferPoolManagerConcurrencyTest, ConcurrencyTest) {
  ...
  for (int run = 0; run < num_runs; run++) {
    ...
    for (int tid = 0; tid < num_threads; tid++) {
      threads.push_back(std::thread([&bpm, fd]() {
        // NOLINT
        ...
        for (int i = 0; i < 10; i++) {
          ...
          strcpy(new_page->get_data(),
                 std::to_string(temp_page_id.page_no).c_str());  // NOLINT
        ...
        for (int j = 0; j < 10; j++) {
          ...
          EXPECT_EQ(0, std::strcmp(std::to_string(page_ids[j].page_no).c_str(),
                                   (page->get_data())));
...
TEST(StorageTest, SimpleTest) {
  ...
  for (size_t i = 0; i < filenames.size(); i++) {
    ...
    char* tmp =
        new char[PAGE_SIZE *
                 MAX_PAGES];  // TODO: fix error in detected memory leaks
...
TEST(RecordManagerTest, SimpleTest) {
  ...
  int record_size =
      4 + rand() % 256;  // 元组大小随便设置，只要不超过RM_MAX_RECORD_SIZE
  // test files
  {
    // 删除残留的同名文件
    if (disk_manager->is_file(filename)) {
    ...
    int max_bytes = file_handle->file_hdr_.record_size *
  ...
  for (int round = 0; round < 1000; round++) {
    ...
    if (mock.empty() || dice < insert_prob) {
      ...
      mock[rid] =
          std::string((char*)write_buf, file_handle->file_hdr_.record_size);
      add_cnt++;
      //            std::cout << "insert " << rid << '\n'; //
      //            operator<<(cout,rid)
    } else {
