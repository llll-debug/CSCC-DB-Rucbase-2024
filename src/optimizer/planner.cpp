...
bool Planner::get_index_cols(std::string& tab_name,
  ...
  index_col_names.reserve(tab.indexes.begin()->second.cols.size());
  ...
  for (auto& [index_name, idx] : repelicate_conds_map) {
    std::ignore = idx;
    fed_conds.emplace_back(
        std::move(curr_conds[repelicate_conds_map[index_name]]));
  }
 ...
 */
std::vector<Condition> Planner::pop_conds(std::vector<Condition>& conds,
  ...
  for (auto&& it = conds.begin(); it != conds.end();) {
    if ((tab_names == it->lhs_col.tab_name &&
         (it->is_rhs_val || it->is_sub_query)) ||
...
std::shared_ptr<Plan> Planner::pop_scan(
    int* scantbl, const std::string& table,
...
std::shared_ptr<Plan> Planner::make_one_rel(std::shared_ptr<Query>& query,
  ...
  for (size_t i = 0; i < tables.size(); i++) {
    ...
    if (index_exist == false) {
      ...
      table_scan_executors[i] = std::make_shared<ScanPlan>(
          T_SeqScan, sm_manager_, std::move(tables[i]), std::move(curr_conds),
          std::move(index_col_names));
    } else {
      // 存在索引
      // 且在排序列上，不需要排序
      // 如果有索引，且是 min，max 聚合直接优化成 orderby +
      // limit，就不用走聚合算子了 select min(no_o_id)/max(no_o_id) as min_o_id
      // from new_orders where no_d_id=:d_id and no_w_id=:w_id; select no_o_id
      // as min_o_id from new_orders where no_d_id=:d_id and no_w_id=:w_id order
      // by no_o_id limit 1;
      if (curr_conds.size() <= index_col_names.size() &&
          query->agg_types.size() == 1) {
      ...
      if (x->has_sort) {
        for (auto& cond : curr_conds) {
          if (cond.lhs_col == query->sort_bys ||
              cond.rhs_col == query->sort_bys) {
            x->has_sort = false;
            break;
          }
        }
      ...
      table_scan_executors[i] = std::make_shared<ScanPlan>(
          T_IndexScan, sm_manager_, std::move(tables[i]), std::move(curr_conds),
          std::move(index_col_names), query->asc);
    }
  }
  ...
  if (!conds.empty()) {
    ...
    while (it != conds.end()) {
      ...
      left = pop_scan(scantbl, it->lhs_col.tab_name, joined_tables,
      ...
      if (left_plan->tag != T_IndexScan) {
        bool index_exist =
            get_index_cols(it->lhs_col.tab_name, join_conds, index_col_names);
        if (index_exist) {
      ...
      if (right_plan->tag != T_IndexScan) {
        bool index_exist =
            get_index_cols(it->rhs_col.tab_name, right_conds, index_col_names);
        if (index_exist) {
          right_plan->tag = T_IndexScan;
          right_plan->conds_.emplace(right_plan->conds_.begin(),
                                     std::move(right_conds[0]));
      ...
      if (x->has_sort) {
        if (left->tag != T_IndexScan && right->tag == T_IndexScan) {
          // 为左列生成 sort
          if (join_conds[0].lhs_col == query->sort_bys ||
              join_conds[0].rhs_col == query->sort_bys) {
            // TODO 检查排序列是否就是连接列，检查排序列上是否有索引
            left = std::make_shared<SortPlan>(
                T_Sort, std::move(left), it->lhs_col,
          ...
          if (join_conds[0].lhs_col == query->sort_bys ||
              join_conds[0].rhs_col == query->sort_bys) {
            // TODO 检查排序列是否就是连接列，检查排序列上是否有索引
            right = std::make_shared<SortPlan>(
                T_Sort, std::move(right), it->rhs_col,
                x->order->orderby_dir == ast::OrderBy_DESC);
            // 不用再生成 sort 算子来排序了
            x->has_sort = false;
          }
      ...
      if (enable_nestedloop_join && enable_sortmerge_join) {
        ...
        table_join_executors =
            std::make_shared<JoinPlan>(T_NestLoop, std::move(left),
                                       std::move(right), std::move(join_conds));
        ...
        if ((left->tag == T_Sort || left->tag == T_IndexScan) &&
          ...
          table_join_executors = std::make_shared<JoinPlan>(
              T_SortMerge, std::move(left), std::move(right),
              std::move(join_conds));
        } else {
          // ticky 把嵌套连接变成归并连接来加速
          // left = std::make_shared<SortPlan>(T_Sort, std::move(left),
          // it->lhs_col,
          //                                   false);
          // right = std::make_shared<SortPlan>(T_Sort, std::move(right),
          // it->rhs_col,
          //                                    false);
          table_join_executors = std::make_shared<JoinPlan>(
              T_NestLoop, std::move(left), std::move(right),
              std::move(join_conds));
        }
      } else {
        // error
...
std::shared_ptr<Plan> Planner::generate_select_plan(
    std::shared_ptr<Query>& query, Context* context) {
  // 逻辑优化
  query = logical_optimization(query, context);
  ...
  if (is_agg) {
    plannerRoot = std::make_shared<AggregatePlan>(
        T_Aggregate, std::move(plannerRoot), std::move(query->cols),
        std::move(query->agg_types), std::move(query->group_bys),
  ...
  return std::make_shared<ProjectionPlan>(
      ...
      std::move(query->alias), query->limit);
}
...
std::shared_ptr<Plan> Planner::do_planner(std::shared_ptr<Query>& query,
  ...
  if (auto x = std::dynamic_pointer_cast<ast::CreateStaticCheckpoint>(
          query->parse)) {
    plannerRoot =
        std::make_shared<StaticCheckpointPlan>(T_CreateStaticCheckpoint);
  } else if (auto x =
                 std::dynamic_pointer_cast<ast::CreateTable>(query->parse)) {
    // create table;
    ...
    plannerRoot = std::make_shared<DDLPlan>(
        T_CreateTable, std::move(x->tab_name), std::vector<std::string>(),
        std::move(col_defs));
  } else if (auto x = std::dynamic_pointer_cast<ast::DropTable>(query->parse)) {
    // drop table;
    plannerRoot = std::make_shared<DDLPlan>(T_DropTable, std::move(x->tab_name),
    ...
    // create index;
    plannerRoot = std::make_shared<DDLPlan>(
        T_CreateIndex, std::move(x->tab_name), std::move(x->col_names),
        std::vector<ColDef>());
  } else if (auto x = std::dynamic_pointer_cast<ast::DropIndex>(query->parse)) {
    // drop index
    plannerRoot = std::make_shared<DDLPlan>(T_DropIndex, std::move(x->tab_name),
                                            std::move(x->col_names),
                                            std::vector<ColDef>());
  } else if (auto x = std::dynamic_pointer_cast<ast::LoadStmt>(query->parse)) {
    // load;
    plannerRoot = std::make_shared<LoadPlan>(T_Load, std::move(x->file_name),
                                             std::move(x->table_name));
  } else if (auto x =
                 std::dynamic_pointer_cast<ast::InsertStmt>(query->parse)) {
    // insert;
    plannerRoot = std::make_shared<DMLPlan>(
        T_Insert, std::shared_ptr<Plan>(), std::move(x->tab_name),
        std::move(query->values), std::vector<Condition>(),
    ...
    if (index_exist == false) {
      ...
      table_scan_executors = std::make_shared<ScanPlan>(
          T_SeqScan, sm_manager_, x->tab_name, std::move(query->conds),
          std::move(index_col_names));
    } else {
      // 存在索引
      table_scan_executors = std::make_shared<ScanPlan>(
          T_IndexScan, sm_manager_, x->tab_name, std::move(query->conds),
          std::move(index_col_names));
    }

    plannerRoot = std::make_shared<DMLPlan>(
        T_Delete, std::move(table_scan_executors), std::move(x->tab_name),
    ...
    plannerRoot = std::make_shared<DMLPlan>(
        T_Update, std::move(table_scan_executors), std::move(x->tab_name),
        std::vector<Value>(), std::vector<Condition>(),
        std::move(query->set_clauses));
  } else if (auto x =
