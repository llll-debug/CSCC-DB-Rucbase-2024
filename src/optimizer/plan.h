...
{
    public:
        FilterPlan(PlanTag tag, std::shared_ptr<Plan> subplan, std::vector<Condition> filter_conds)
        {
            Plan::tag = tag;
            subplan_ = std::move(subplan);
        ...
        ~FilterPlan(){}
        std::shared_ptr<Plan> subplan_;
        std::vector<Condition> filter_conds_;  // WHERE条件
};
...
{
public:
    ExplainPlan(PlanTag tag, std::shared_ptr<PlanTreeNode> plan_tree)
    {
        Plan::tag = tag;
    ...
    std::shared_ptr<PlanTreeNode> plan_tree_;
};
