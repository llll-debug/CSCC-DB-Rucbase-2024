...
int main() {
  ...
  std::vector<std::string> aggSqls = {
      "select v1, count(*) as v2, count(v1) as v3, sum(v1) as v4, max(v1) as "
      "v5, min(v1) as v6 from t1;",
      "select id,MAX(score) as max_score,MIN(score) as min_score,SUM(score) as "
      "sum_score from grade group by id;",
      "select id,MAX(score) as max_score from grade group by id, course having "
      "v1 > 0;",  // 需要手动检查
      "select id, MAX(score) as max_score from grade where MAX(score) > 90 "
      "group by id;"  // 语法分析报错时抛出即可
  };
  std::vector<std::string> subquerySqls = {
      ...
      "select id from grade where score < (select MAX(score) from grade);",
      "select id from grade where score = (select MIN(score) from grade where "
      "score > (select MAX(score) from grade));",
      // 多级嵌套
      "select id from grade where name in (select name from grade);",
      "select id from grade where score = (select MIN(score) from grade where "
      "score in (select MAX(score) from grade));"};
  ...
  std::vector<std::string> PerformanceSqls = {
      // 为了减少编写词法规则工作量，在分析阶段判断非法 sqls
      "load ../../src/test/performance_test/table_data/warehouse.csv into "
      "warehouse;",
