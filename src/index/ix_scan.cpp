...
void IxScan::next() {
  ...
  if (iid_.page_no != ih_->file_hdr_->last_leaf_ &&
      iid_.slot_no == cur_node_handle_->get_size()) {
    // go to next leaf
    iid_.slot_no = 0;
...
Rid IxScan::prev_rid(const Iid& iid) { return ih_->get_rid(prev_iid(iid)); }

RmRecord IxScan::get_key() { return ih_->get_key(iid_); }
